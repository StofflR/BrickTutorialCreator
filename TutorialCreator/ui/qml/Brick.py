...
class Brick(QObject):
    ...
    def updateBrick(self, color, path, size, content, scale):
        if color and path and size  and scale:
            self.brick = SVGBrick(base_type=color, content=content, size=size, path=path, scaling_factor=scale / 100)
    ...
    def updateBrickContent(self, content):
        ...
        if color and path and size  and scale:
            self.brick = SVGBrick(base_type=color, content=content, size=size, path=path, scaling_factor=scale)
    ...
    @Slot(result=str)
    def fileName(self):
        return ' '.join(self.brick.contentPlain().split()).strip().replace(" ", "_") #clean multi, leading/tailing whitespaces
